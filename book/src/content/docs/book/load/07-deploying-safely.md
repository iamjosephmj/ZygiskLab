...
always relabelling rather than for testing whether you need to. Verify with:

```bash
adb shell su -M -c 'ls -Z /data/adb/modules/<id>/zygisk/'
```

and compare against the label on a module you did not deploy by hand.
...
Every deploy, in order. `deploy.sh` does the first five for you; the point of
listing them is that you can diagnose a deploy that goes wrong.

1. Build, and confirm the `.so` timestamp is from this build, not the last one.
2. `adb push` to `/data/local/tmp`.
3. `su -M -c mv` into the module directory. Never `cp`.
4. `chmod 644` and `restorecon` (or `chcon --reference`) on the destination.
5. `md5sum` both sides and compare before you touch anything else.
6. `adb reboot`. Then clear the log and reproduce.
7. If an armed app crashes, hash first, reboot second, debug third.
...
bugs. The hash tells them apart in two seconds, and only if you check it before
you have already convinced yourself of a story.
